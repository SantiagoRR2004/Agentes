...
+!get(drug) : .my_name(Name) <- 
   ...
   .wait(Time);
   .send(robot, achieve, has(Name, drug)).
...
+!check_bored : true
   <- .random(X); .wait(X*5000+2000);  // Owner get bored randomly
      .send(robot, askOne, time(_), R); // when bored, owner ask the robot about the time
      .print(R);
	  .send(robot, tell, chat("What's the weather in Ourense?"));
      !check_bored.
