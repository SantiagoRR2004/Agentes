...
public class HouseModel extends GridWorldModel {
  ...
  */
  ...
  // Get a random valid location for an agent
  private Location getRandomValidLocation(Random rand, int agentId) {
    ...
    if (agentId == 0) {
      ...
      return new Location(GSize / 2 + 2, GSize - 3); // owner default
    }
  }
  ...
  private List<Location> findPath(int Ag, Location start, Location goal) {
    ...
    while (!openSet.isEmpty()) {
      ...
      for (int[] dir : directions) {
        ...
        PathNode neighbor =
            new PathNode(neighborLoc, current, tentativeG, neighborLoc.distance(goal));
  ...
  boolean moveTowards(int Ag, Location dest) {
    ...
    if (path == null
        || path.isEmpty()
        || !currentPos.equals(
            path.isEmpty() ? currentPos : (path.size() == 1 ? currentPos : currentPos))) {
