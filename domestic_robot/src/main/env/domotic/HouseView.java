...
public class HouseView extends GridWorldView {
  ...
  public HouseView(HouseModel model) {
    super(model, "Domestic Care Robot", Toolkit.getDefaultToolkit().getScreenSize().width / 4);
    hmodel = model;
    // viewSize = model.GridSize;
    // setSize(viewSize, viewSize/2);
    setSize(
        Toolkit.getDefaultToolkit().getScreenSize().width * 3 / 7,
        Toolkit.getDefaultToolkit().getScreenSize().height * 3 / 7);
  ...
  public void draw(Graphics g, int x, int y, int object) {
    ...
    switch (object) {
      ...
      case HouseModel.DOOR:
        g.setColor(Color.lightGray);
        if (lRobot.equals(loc)
            | lRobot.isNeigbour(loc)
            | lOwner.equals(loc)
            | lOwner.isNeigbour(loc)) { // |
      ...
      case HouseModel.CHARGER:
        g.setColor(Color.lightGray);
        if (lRobot.equals(hmodel.lCharger)) {
          objPath = "/doc/botCharging.png"; // currentDirectory.concat("/doc/openWasher.png");
          drawImage(g, x, y, objPath);
          // super.drawAgent(g, x, y, Color.red, -1);
        } else {
          objPath = "/doc/Charger.png"; // currentDirectory.concat("/doc/closeWasher.png");
          drawImage(g, x, y, objPath);
  ...
  public void drawAgent(Graphics g, int x, int y, Color c, int id) {
    ...
    if (id < 1) {
      if (!lRobot.equals(lOwner)
          && !lRobot.equals(hmodel.lFridge)
          && !lRobot.equals(hmodel.lCharger)) {
        c = Color.yellow;
  ...
  public void drawMultipleImage(Graphics g, int x, int y, String imageAddress, int NW, int NH) {
    ...
    if (url == null) System.out.println("Could not find image! " + imageAddress);
  ...
  public void drawMultipleScaledImage(
      Graphics g, int x, int y, String imageAddress, int NW, int NH, int scaleW, int scaleH) {
    URL url = getClass().getResource(imageAddress);
    ImageIcon Img = new ImageIcon();
    if (url == null) System.out.println("Could not find image! " + imageAddress);
    else Img = new ImageIcon(getClass().getResource(imageAddress));
    ...
    g.drawImage(
        Img.getImage(),
        x * cellSizeW + NW * cellSizeW * (100 - scaleW) / 200,
        y * cellSizeH + NH * cellSizeH * (100 - scaleH) / 200 + 1,
        NW * cellSizeW * scaleW / 100,
        NH * scaleH * cellSizeH / 100,
        null);
  }

  public void drawScaledImage(
      Graphics g, int x, int y, String imageAddress, int scaleW, int scaleH) {
    URL url = getClass().getResource(imageAddress);
    ImageIcon Img = new ImageIcon();
    if (url == null) System.out.println("Could not find image!" + imageAddress);
    else Img = new ImageIcon(getClass().getResource(imageAddress));
    ...
    g.drawImage(
        Img.getImage(),
        x * cellSizeW + cellSizeW * (100 - scaleW) / 200,
        y * cellSizeH + cellSizeH * (100 - scaleH) / 100,
        cellSizeW * scaleW / 100,
        scaleH * cellSizeH / 100,
        null);
  }

  public void drawScaledImageUp(
      Graphics g, int x, int y, String imageAddress, int scaleW, int scaleH) {
    URL url = getClass().getResource(imageAddress);
    ImageIcon Img = new ImageIcon();
    if (url == null) System.out.println("Could not find image! " + imageAddress);
  ...
  public void drawScaledImageLf(
    ...
    g.drawImage(
        Img.getImage(),
        x * cellSizeW,
        y * cellSizeH + cellSizeH * (100 - scaleH) / 200 + 1,
        cellSizeW * scaleW / 100,
        scaleH * cellSizeH / 100,
        null);
  }

  public void drawScaledImageRt(
      Graphics g, int x, int y, String imageAddress, int scaleW, int scaleH) {
    URL url = getClass().getResource(imageAddress);
    ImageIcon Img = new ImageIcon();
    if (url == null) System.out.println("Could not find image! " + imageAddress);
    ...
    g.drawImage(
        Img.getImage(),
        x * cellSizeW + cellSizeW * (100 - scaleW) / 100,
        y * cellSizeH + cellSizeH * (100 - scaleH) / 200 + 1,
        cellSizeW * scaleW / 100,
        scaleH * cellSizeH / 100,
        null);
  ...
  public void drawMan(Graphics g, int x, int y, String how) {
    ...
    // ImageIcon Img = new ImageIcon(getClass().getResource(resource));
    g.drawImage(
        Img.getImage(), x * cellSizeW + 1, y * cellSizeH + 1, cellSizeW - 3, cellSizeH - 3, null);
  }
  ...
  public void drawManSittingRight(Graphics g, int x, int y) {
    ...
    ImageIcon Img = new ImageIcon();
    if (url == null) System.out.println("Could not find image! " + objPath);
    else Img = new ImageIcon(getClass().getResource(objPath));
    // ImageIcon Img = new ImageIcon(getClass().getResource(objPath));
    g.drawImage(
        Img.getImage(), x * cellSizeW - 4, y * cellSizeH + 1, cellSizeW + 2, cellSizeH - 2, null);
  }
